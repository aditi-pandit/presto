...
#include "presto_cpp/main/functions/bloom_filter/BloomFilterFunctionsRegistrations.h"
...
namespace facebook::presto::aggregate::test {
namespace {
...
static bool bloomContains(const velox::StringView& raw, uint64_t hash) {
  ...
  auto* blocks = static_cast<velox::SplitBlockBloomFilter::Block*>(
      ::aligned_alloc(kBlockSize, numBytes));
  std::memcpy(blocks, raw.data(), numBytes);
  const bool result =
      velox::SplitBlockBloomFilter(
...
class BloomFilterAggregateFunctionTest : public AggregationTestBase {
 protected:
  void SetUp() override {
    ...
    facebook::presto::functions::bloom_filter::
        registerAllBloomFilterFunctions();
  }

  /// Runs bloom_filter_agg over a single column and returns the result bytes.
  /// Asserts that the result is non-null and non-empty.
  velox::StringView runBloomFilterAgg(const velox::VectorPtr& column) {
    ...
    auto* flat = result->childAt(0)->asFlatVector<velox::StringView>();
...
TEST_F(BloomFilterAggregateFunctionTest, varchar) {
  auto values = makeFlatVector<velox::StringView>({"hello", "world"});
  auto raw = runBloomFilterAgg(values);
...
TEST_F(BloomFilterAggregateFunctionTest, varbinary) {
  auto values = makeFlatVector<velox::StringView>(
      {velox::StringView("\x01\x02"), velox::StringView("\x03\x04")},
      velox::VARBINARY());
  auto raw = runBloomFilterAgg(values);
...
TEST_F(BloomFilterAggregateFunctionTest, nullInputsIgnored) {
  auto values =
      makeNullableFlatVector<int64_t>({std::nullopt, 7LL, std::nullopt, 8LL});
  auto raw = runBloomFilterAgg(values);
...
TEST_F(BloomFilterAggregateFunctionTest, allNullInputReturnsNull) {
  auto values = makeNullableFlatVector<int64_t>({std::nullopt, std::nullopt});
...
TEST_F(BloomFilterAggregateFunctionTest, partialFinalGroupBy) {
  ...
  auto plan =
      PlanBuilder()
          .values(
              {makeRowVector({keys1, vals1}), makeRowVector({keys2, vals2})})
  ...
  auto* filterCol = result->childAt(1)->asFlatVector<velox::StringView>();
