...
class ExcludeColumns : public TableFunction {
 public:
  explicit ExcludeColumns(velox::memory::MemoryPool* pool)
  ...
  static std::unique_ptr<ExcludeColumnsAnalysis> analyze(
    ...
    std::vector<velox::TypePtr> returnTypes;
    for (velox::column_index_t i = 0; i < inputColumns.size(); i++) {
