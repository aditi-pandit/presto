...
Table-Valued Functions (TVFs) are SQL functions that return tables rather than scalar values. They appear in the `FROM` clause and can accept other tables, descriptors, and scalar constants as arguments. This document covers the **C++ native worker SPI** and provides
worked examples for both execution modes.
